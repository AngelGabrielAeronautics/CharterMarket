...
import { Paper, TextField, Box, Button, InputAdornment, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, Checkbox, GlobalStyles, IconButton, Typography } from '@mui/material';
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider';
import { AdapterDateFns } from '@mui/x-date-pickers/AdapterDateFns';
import { DatePicker, DatePickerToolbar, DatePickerToolbarProps } from '@mui/x-date-pickers/DatePicker';
import { DateTimePicker, DateTimePickerToolbar, DateTimePickerToolbarProps } from '@mui/x-date-pickers/DateTimePicker';
...
export default function BookingForm() {
  ...
  const handleIncrementPassengers = () => {
    setSearchForm(prev => {
      ...
      if (flightType === 'one-way' && prev.departureDate && newPax !== '-') {
        setOneWayStep(3);
      } else if (flightType === 'return' && prev.departureDate && prev.returnDate && newPax !== '-') {
        setReturnStep(4);
      }
  ...
  const handleMultiCityLegChange = (legIndex: number, field: keyof typeof searchForm.multiCityLegs[0], value: any) => {
    setSearchForm(prev => {
  ...
  const CustomPickerLayout = forwardRef<HTMLDivElement, PickersLayoutProps<Date | null>>((props, ref) => {
    ...
    const onEstimateChange = (ownerState as any).onEstimateChange as (event: React.ChangeEvent<HTMLInputElement>) => void;
    
    return (
      <PickersLayoutRoot ref={ref} className={pickersLayoutClasses.root} ownerState={ownerState} sx={{ display: 'flex', flexDirection: 'column' }}>
        <PickersLayoutContentWrapper 
        ...
        <Box sx={{ px: 2, py: 1, borderTop: `1px solid ${theme.palette.divider}`, borderBottom: `1px solid ${theme.palette.divider}`, flexGrow: 0, flexShrink: 0 }}>
        <FormControlLabel
          control={
            <Checkbox
              size="small"
  ...
  const CustomActionBar = (props: AugmentedActionBarProps) => {
    ...
    return (
      ...
      >
        <Button onClick={() => customOnReset?.()} color="inherit">Reset</Button>
        <Box>
          <Button onClick={() => customOnCancelAndReset?.()} color="inherit" sx={{ mr: 1 }}>Cancel</Button>
          <Button onClick={() => customOnClose?.()} variant="contained">OK</Button>
        </Box>
      </Box>
  );
  };
      ...
      <Paper
        ...
        >
          ...
          <Autocomplete
            ...
            sx={{ flex: '1 1 180px', minWidth: 120 }}
          />
          <Autocomplete
            freeSolo
            options={airportOptions}
            ...
            <Box sx={{ cursor: 'pointer', flex: '1 1 150px', minWidth: 150 }}>
              <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={activeLocale}>
                <DateTimePicker
                  ...
                  views={searchForm.multiCityLegs[0].departureDateEstimate ? ['year', 'month'] : ['year', 'month', 'day', 'hours', 'minutes']}
                  ...
                  slots={{ layout: CustomPickerLayout, actionBar: CustomActionBar, toolbar: undefined }}
                  slotProps={{
                    actionBar: {
                      customOnClose: () => handleCloseMultiCityPicker(0),
                      customOnReset: () => handleMultiCityLegChange(0, 'departureDate', initialMultiCityDates[0]),
                    ...
                    layout: {
                      ...
                      onEstimateChange: (e: React.ChangeEvent<HTMLInputElement>) => handleMultiCityLegChange(0, 'departureDateEstimate', e.target.checked)
                    } as any
                  }}
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: theme.spacing(2) }}>
              <TextField
                ...
                InputProps={{
                  ...
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton onClick={handleDecrementPassengers} size="small" disabled={searchForm.passengers === '-'}>
                        <RemoveIcon fontSize="small" />
                      </IconButton>
                      <IconButton onClick={handleIncrementPassengers} size="small" disabled={searchForm.passengers === '500'}>
                ...
                sx={{
                  ...
                  maxWidth: '250px',
                  '& .MuiOutlinedInput-root': {
                    backgroundColor: searchForm.passengers && searchForm.passengers !== '-' ? '#fdfaf6' : 'transparent',
                    '&:hover': {
                      backgroundColor: theme.palette.action.hover,
                      '& .MuiOutlinedInput-notchedOutline': {
                        borderColor: theme.palette.mode === 'light' ? 'rgba(0, 0, 0, 0.23)' : 'rgba(255, 255, 255, 0.23)',
              ...
              <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={activeLocale}>
                <DateTimePicker
                  ...
                  onChange={(newDate) => {
                    ...
                    if (newDate) setOneWayStep(2); else setOneWayStep(1);
                  }}
                  minDateTime={new Date()}
                  maxDateTime={addMonths(new Date(), 18)}
                  views={searchForm.oneWayDepartureDateEstimate ? ['year', 'month'] : ['year', 'month', 'day', 'hours', 'minutes']}
                  ...
                  slots={{ 
                    ...
                    textField: (propsFromPicker) => {
                      const originalFormattedValue = typeof propsFromPicker.value === 'string' ? propsFromPicker.value : '';
                      const displayValue = originalFormattedValue.endsWith(" 00:00") 
                        ? originalFormattedValue.substring(0, originalFormattedValue.length - " 00:00".length) + " --:--"
                        : originalFormattedValue;
                      const handleDisplayChange = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
                        let textFromInput = event.target.value;
                        if (textFromInput.endsWith(" --:--")) {
                          textFromInput = textFromInput.substring(0, textFromInput.length - " --:--".length) + " 00:00";
                        }
                        const modifiedEvent = { ...event, target: { ...event.target, value: textFromInput } };
                        if (propsFromPicker.onChange) {
                          propsFromPicker.onChange(modifiedEvent as React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>);
                        }
                      };
                      return <TextField {...propsFromPicker} value={displayValue} onChange={handleDisplayChange} sx={propsFromPicker.sx} />;
                    }
                  }}
                  slotProps={{
                    actionBar: { 
                      ...
                      customOnReset: () => {
                        ...
                        if (!initialOneWayDate) setOneWayStep(1); else setOneWayStep(2);
                      },
                      customOnCancelAndReset: () => {
                        setSearchForm(prev => ({ ...prev, departureDate: initialOneWayDate }));
                        setOpenOneWay(false);
                    ...
                    layout: { 
                      ...
                      onEstimateChange: (e: React.ChangeEvent<HTMLInputElement>) => 
                        setSearchForm(prev => ({ ...prev, oneWayDepartureDateEstimate: e.target.checked }))
                    } as any
                  }}
                  enableAccessibleFieldDOMStructure={false}
                />
        ...
        {flightType === 'return' && (
          <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: theme.spacing(2) }}>
            <Box sx={{ cursor: 'pointer' }}>
              <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={activeLocale}>
                <DateTimePicker
                  label="Departure Date"
                  value={searchForm.departureDate}
                  onChange={(newDate) => {
                    setSearchForm(prev => ({ ...prev, departureDate: newDate, returnDate: null }));
                    if (newDate) setReturnStep(2); else setReturnStep(1);
                  }}
                  minDateTime={new Date()}
                  maxDateTime={addMonths(new Date(), 18)}
                  views={searchForm.returnDepartureDateEstimate ? ['year', 'month'] : ['year', 'month', 'day', 'hours', 'minutes']}
                  ...
                  slotProps={{
                    actionBar: { 
                      ...
                      customOnReset: () => {
                        setSearchForm(prev => ({ ...prev, departureDate: initialReturnDepartureDate }));
                        if (!initialReturnDepartureDate) setReturnStep(1); else setReturnStep(2);
                      },
                      customOnCancelAndReset: () => {
                        setSearchForm(prev => ({ ...prev, departureDate: initialReturnDepartureDate }));
                    ...
                    layout: { 
                      ...
                      onEstimateChange: (e: React.ChangeEvent<HTMLInputElement>) => 
                        setSearchForm(prev => ({ ...prev, returnDepartureDateEstimate: e.target.checked }))
                    } as any
                  }}
                  enableAccessibleFieldDOMStructure={false}
                />
              ...
              <Box sx={{ cursor: 'pointer' }}>
                <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={activeLocale}>
                  <DateTimePicker
                    ...
                    maxDateTime={addMonths(new Date(), 18)}
                    views={searchForm.returnReturnDateEstimate ? ['year', 'month'] : ['year', 'month', 'day', 'hours', 'minutes']}
                    ...
                    slotProps={{
                      actionBar: { 
                        ...
                        customOnReset: () => {
                          setSearchForm(prev => ({ ...prev, returnDate: initialReturnReturnDate }));
                      ...
                      layout: { 
                        ...
                        onEstimateChange: (e: React.ChangeEvent<HTMLInputElement>) => 
                          setSearchForm(prev => ({ ...prev, returnReturnDateEstimate: e.target.checked }))
                      } as any
                    }}
                    enableAccessibleFieldDOMStructure={false}
        ...
        {flightType === 'return' && (
          <Box sx={{ width: '100%', mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {returnStep >= 4 && searchForm.departureDate && searchForm.returnDate && searchForm.passengers && searchForm.passengers !== '-' && (
        ...
        {flightType === 'multicity' && (
          <Box sx={{ width: '100%', mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {searchForm.multiCityLegs.map((leg, legIndex) => legIndex > 0 && (
              <Box
                key={`leg-${legIndex}`} 
                sx={{ 
                  display: 'flex', 
              ...
              >
                ...
                {((legIndex === 0 && searchForm.from && searchForm.to) || (legIndex > 0 && leg.from && leg.to)) && (
                  <Box sx={{ cursor: 'pointer', flex: '1 1 150px', minWidth: 150 }}>
                    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={activeLocale}>
                      <DateTimePicker
                        label="Departure Date"
                        ...
                        onChange={(newDate) => {
                          ...
                          if (newDate) {
                            ...
                            const isAnyLegComplete = searchForm.multiCityLegs.some((l, idx) => 
                        ...
                        minDateTime={new Date()}
                        maxDateTime={addMonths(new Date(), 18)}
                        views={leg.departureDateEstimate ? ['year', 'month'] : ['year', 'month', 'day', 'hours', 'minutes']}
                        ...
                        slotProps={{
                          actionBar: { 
                            ...
                            customOnReset: () => {
                              handleMultiCityLegChange(legIndex, 'departureDate', initialMultiCityDates[legIndex]);
                          ...
                          layout: { 
                            ...
                            onEstimateChange: (e: React.ChangeEvent<HTMLInputElement>) => 
                              handleMultiCityLegChange(legIndex, 'departureDateEstimate', e.target.checked)
                          } as any
                        }}
                        enableAccessibleFieldDOMStructure={false}
                      />
              ...
              >
                <FormControlLabel
                  ...
                  control={
                    <Radio
                      icon={
                        <Box sx={{ p: 1, borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <ArrowForwardIcon color="action" />
                        </Box>
                      }
                      checkedIcon={
                        <Box sx={{ p: 1, borderRadius: '50%', border: '2px solid', borderColor: 'primary.main', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fdfaf6' }}>
